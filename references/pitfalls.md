...
1. Generate slides -> Extract text with `python -m markitdown output/presentation.pptx` -> Review content
2. **List issues found** (if none found, look again more critically)
3. Fix issues
4. **Re-verify affected slides** — one fix often creates another problem
5. Repeat until a full pass reveals no new issues

**Footer / confidential mark check**: If your template embeds a footer or confidential mark on specific layouts, close the deck with one of those layouts and verify the mark appears at the expected position. Don't draw a custom mark on top of a template-provided one, or it may appear twice. The bundled default template has no such mark — if you need one, draw it explicitly using `theme.json` colors.
